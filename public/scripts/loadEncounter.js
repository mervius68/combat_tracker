...
async function load_encounter(encounterCode = 0, dataNav = 1, getCtApp = true) {
    ...
    let encounter = await dbQuery("GET", "selected_encounter/" + encounterID);
    let header = document.querySelector(".main_header");
    header.textContent = `${encounter[0]?.campaign || "No available encounters yet"} - ${encounter[0]?.location || ""} (${encounter[0]?.description || ""})`;
    ...
    let ctConditions = await dbQuery("GET", "getConditionsForCtApp/" + encounterID);
    ...
    participantsHpsByRound = await dbQuery("GET", "hpsByRound/" + encounterID);
    ...
    ctApp.forEach((participant) => {
        ...
        for (let i = 0; i < damageRounds; i++) {
            ...
            let x = participantsHpsByRound.filter((target) => {
                return (target.result_round == i + 1 &&
    ...
    ctApp.forEach((participant) => {
        ...
        ctConditions.forEach((condition) => {
            ...
            participant.conditionsArray =
                participant.conditionsArray.filter((obj, index, self) => index ===
    ...
    for (let i = 1; i <= totalRounds + 1; i++) {
        ...
        const filterActionsByType = (ctActions, ctApp, actionType, round) => ctApp.map((participant) => ctActions.filter((action) => action.pID === participant.pID &&
        ...
        let sectionHTML = buildASection("attack", ctRound, numAttacks, ctApp);
        mainContainer.appendChild(sectionHTML);
        // build the bonus section if there is one, empty.
        sectionHTML = buildASection("bonus", ctRound, bonusActions, ctApp);
        mainContainer.appendChild(sectionHTML);
        // build the reaction section if there is one, empty
        sectionHTML = buildASection("react", ctRound, reactActions, ctApp);
        mainContainer.appendChild(sectionHTML);
        ...
        ctApp.forEach(async (participant) => {
            ...
            let roundDamages = ct_damages.filter((item) => {
                return (item.round == i && item.target_pID == participant.pID);
            });
            ...
            participantHTML = mainContainer.querySelectorAll('[data-round="' +
                i +
            ...
            participantHTML.forEach((turn, index) => {
                ...
                participant.conditionsArray.forEach(async (condition) => {
                    let affecteesStill = await dbQuery("GET", "anyoneStillAffected/" +
                        condition.conditionID +
                        "/" +
                        i);
                    const maxValue = Math.max(...affecteesStill.map((obj) => obj.end_round));
                    const maxObjects = affecteesStill.filter((obj) => obj.end_round === maxValue);
                    let affectedThisRound = ctApp.filter((item) => affecteesStill.some((obj) => obj.affected_pID === item.pID));
                    let endRound = maxObjects[0]?.end_round;
                    let affectedInitLower = false;
                    if (affectedThisRound.some((item) => item.init < participant.init)) {
                    ...
                    for (let i = ctApp.length - 1; i >= 0; i--) {
                        ...
                        matchingObject = ctApp.find((obj) => obj.pID === value);
                        // If a matching object is found, exit the loop
                        if (matchingObject) {
                            break;
                        }
                    }
                    if (((parseInt(condition.concentration) == 1 || parseInt(condition.holding) == 1) &&
                        ...
                        let affectees = await dbQuery("GET", "getAffectees/" + condition.taID + "/" + i);
                        let affecteesString = "";
                        ...
                        if (condition.row_start <= row && condition.row_stop >= row) {
                            ...
                            toolTipHTML.setAttribute("data-condition-id", condition.conditionID);
                            toolTipHTML.setAttribute("data-affected-id", condition.affected_pID);
                            toolTipHTML.setAttribute("data-taid", condition.taID);
                            toolTipHTML.setAttribute("data-causerID", condition.pID);
                ...
                participant.affectedArray.forEach((affected) => {
                    const affectedParticipant = ctApp.find((participant) => participant.pID === affected.pID);
                    ...
                    if (affected.concentration === 0
                        ...
                        else if (affectedPosition == endPosition &&
                            affectedPosition < causerPosition &&
                            starter == i) {
                        }
                        else if (endPosition <= affectedPosition &&
                            ((starter == i && stopper > i) ||
                                (endPosition == affectedPosition &&
                                    stopper == i))) {
                            populateAffected();
                        }
                        else if (affectedPosition <= causerPosition &&
                            starter < i &&
                            stopper == i) {
                            populateAffected();
                        }
                        else if (causerPosition < affectedPosition &&
                        ...
                        function populateAffected() {
                            ...
                            if (affected.row_start <= row && affected.row_stop >= row) {
                                ...
                                toolTipHTML.setAttribute("data-condition-id", affected.conditionID);
                                toolTipHTML.setAttribute("data-affected-id", affected.affected_pID);
                                toolTipHTML.setAttribute("data-this-round", i);
                                toolTipHTML.innerHTML = "A";
                                toolTipHTML.classList.add("hovertwin");
                                toolTipHTML.setAttribute("data-taid", affected.taID);
                                toolTipHTML.setAttribute("data-hover-id", affected.taID);
                                ...
                                turn.nextSibling.nextSibling.appendChild(toolTipHTML);
                            }
                        }
                    }
                });
            ...
            participantHTML.forEach(async (turn, index) => {
                ...
                let x = turn.nextSibling.nextSibling.querySelectorAll(".affected");
                ...
                if (lastNum <= participant.max_hp / 2) {
                    ...
                    turn.nextSibling.nextSibling.nextSibling.classList.add("yellow");
                }
                if (lastNum <= 0) {
                    ...
                    turn.nextSibling.nextSibling.nextSibling.classList.add("red");
                }
            });
        });
    }
    ...
    try {
        for (const action of ctActions) {
            ...
            let targets, targetString = "", damageString = "";
            try {
                targets = await dbQuery("GET", "targets/" + target[0].targetID);
                targetString = "";
                damageString = "";
            ...
            await sendToCoordinate(action.round, action.pID, action.action_type || "", actionObject, action.taID || "0");
        }
    }
    catch (err) { }
    // size the sections
    ...
    function sendToCoordinate(round, participant, actionType, actionObject, taID) {
        try {
            let y = mainContainer.querySelector(`[data-round="${round}"] [data-section="2"] [div-participant="${participant}"]`);
            let y_drop = y;
            let y_start = y.nextSibling;
            let y_notes = y.nextSibling.nextSibling.nextSibling;
            let y_end = y.nextSibling.nextSibling;
            if ((actionObject.notes && actionObject.notes != "-")) {
                y_notes.innerHTML =
    ...
    try {
        ...
        startNav.parentNode.parentNode.previousSibling.classList.remove("hidden");
    }
    catch (err) { }
    ...
    const contextMenuListener = function (e) {
        ...
        if (element.classList.contains("affected") || element.classList.contains("concentration")) {
            ...
            cmEnd.setAttribute("data-cm-condition-id", element.getAttribute("data-condition-id"));
            cmEnd.setAttribute("data-cm-causer-id", element.getAttribute("data-causerID"));
            cmEnd.setAttribute("data-cm-affected-id", element.getAttribute("data-affected-id"));
            cmEnd.setAttribute("data-cm-taid", element.getAttribute("data-taid"));
            cmEnd.setAttribute("data-cm-round", element.getAttribute("data-this-round"));
            cmEnd.setAttribute("data-cm-concentration-or-affected", element.classList.contains("affected")
                ? "affected"
                : "concentration");
            showContextMenu();
        }
